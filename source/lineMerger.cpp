...
namespace mac
{
    ...
    void lineMerger_t::processLines()
    {
        ...
        for (const auto &hey : m_slopeGroups)
        {
            std::cout << hey.first << " " << hey.second.lines_to_be_added[0].left.x << " " << hey.second.lines_to_be_added[0].left.y;
        }
    ...
    void lineMerger_t::updateSlopeGroups(const std::vector<mac::modifiedSlopeGroup_t> &slopeGroup_vec)
    {
    ...
    void lineMerger_t::updateSlopeGroup(mac::slopeGroup_t &slopeGroup)
    {
